...
RED  = "\033[1;91m"       # bold red            (logo art)
...
# ── Logo ──────────────────────────────────────────────────────────────────────
# SERK art is 32 chars wide. Left box = 34 (32 + 1 space each side).
# Right box = PROXY/SCRAPER/VERSION — same as original MAHDI style.
_L1 = "\033[38;5;47m"   # logo art green 1
_L2 = "\033[38;5;49m"   # logo art green 2
_L3 = "\033[38;5;50m"   # logo art green 3

LOGO = (
    f"{G2}╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\n"
    f"{G2}║{RST} {_L1}.d8888.  d88888b d8888b. db   dD {G2}║\n"
    f"{G2}║{RST} {_L1}88'  YP  88'     88  `8D 88 ,8P' {G2}║\n"
    f"{G2}║{RST} {_L2}`8bo.    88ooooo 88oobY' 88,8P   {G2}║\n"
    f"{G2}║{RST} {_L2}  `Y8b.  88ooooo 88`8b   88`8b   {G2}║\n"
    f"{G2}║{RST} {_L3}db   8D  88.     88 `88. 88 `88. {G2}║\n"
    f"{G2}║{RST} {_L3}`8888Y'  Y88888P 88   YD YP   YD {G2}║\n"
    f"{G2}╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝\n"
    f"{G1}⋆{G2}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{G1}⋆{RST}\n"
    f" {BOLD}[{G1}⍣{BOLD}]{G3} OWNER     : SERK{RST}\n"
    f" {BOLD}[{G1}⍣{BOLD}]{G3} GITHUB    : MAHDI{RST}\n"
    f" {BOLD}[{G1}⍣{BOLD}]{G4} VERSION   : 1.1{RST}\n"
    f"{G1}⋆{G2}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{G1}⋆{RST}"
)
...
def setup() -> dict | None:
    ...
    username = input("[?] GitHub username: \033[0m").strip()
    token    = input("[?] GitHub token (repo scope): \033[0m").strip()
    ...
    create = input("[?] Create a new repository? (y/n): \033[0m").lower()
    if create == "y":
        repo = input("[?] Repository name [proxies]: \033[0m").strip() or "proxies"
    else:
        repo = input("[?] Existing repository name: \033[0m").strip()
        if not repo:
...
def change_link():
    ...
    token    = input("[?] New token: \033[0m").strip()
...
def update_proxies():
    clear()
    # Load config only if already set up — don't force it
    config = load_config()  # may be None — that's fine until push time
    ...
    intensity = input("[?] Choose (1/2/3): \033[0m").strip()
    ...
    fc = input("[?] Filter (1/2/3/4): \033[0m").strip()
    ...
    # Plain ip:port format — no protocol prefix
    ...
    choice = input("[?] (A/M/S): \033[0m").upper().strip()

    if choice == "M":
        print(f"\nPREVIEW (first 30):{RST}")
        linex()
        for p in working[:30]:
            print(f"  {p['protocol']:<6} {p['proxy']} — {p['speed']}s{RST}")
        if len(working) > 30:
        ...
        print(f"║  [X] Discard                                   ║")
        print(f"╚══════════════════════════════════════════════════╝{RST}")
        choice = input("[?] (A/S/X): \033[0m").upper().strip()
        if choice == "X":
...
def show_link():
    ...
    try:
        ...
        if r.status_code == 200:
            count = len([l for l in r.text.splitlines() if re.match(r'\d+\.\d+\.\d+\.\d+:\d+', l)])
...
def main():
    while True:
        ...
        if cfg:
            ...
            gh_status = f"{RED}✗ Not configured{RST}"

        print(f"{YEL}  [1] HARVEST PROXIES  (no GitHub needed)")
        print(f"{YEL}  [2] SETUP GITHUB{RST}     ({gh_status})")
        print(f"{YEL}  [3] SHOW MY LINK")
        print(f"{YEL}  [4] CHANGE GITHUB SETTINGS")
        print(f"{YEL}  [0] EXIT{RST}")
        linex()

        choice = input("[?] Choice: \033[0m").strip()
        if choice == "1":
            update_proxies()
        elif choice == "2":
            if cfg:
